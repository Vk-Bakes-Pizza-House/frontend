...
// ── Reusable Input ────────────────────────────────────────────
function Input({ label, hint, error, type = "text", value, onChange, placeholder, disabled, readOnly, icon: Icon }) {
  return (
    <div className="flex flex-col gap-1.5">
      {label && <label className="text-xs font-semibold text-stone-400 tracking-wider uppercase">{label}</label>}
      <div className="relative">
        {Icon && <div className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-500"><Icon size={14} /></div>}
        <input
          type={type}
          value={value}
          onChange={onChange}
          ...
          className={`
            ...
            ${disabled || readOnly
              ? "bg-stone-800/50 border-stone-700 text-stone-500 cursor-default"
              : "bg-stone-800 border-stone-600 text-stone-100 focus:border-orange-500 focus:ring-1 focus:ring-orange-500/30"
            }
